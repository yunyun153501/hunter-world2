...
> ⚠️ **세금은 페르소나 전용**: 소득 기록은 페르소나 활성화 상태에서만 생성되며, 납부도 페르소나만 가능

| 항목 | 내용 |
|------|------|
| **납부 주체** | **페르소나 전용** (캐릭터/공용은 납부 불가) |
| **납부 기한** | 매월 1~10일 (전월 소득분) |
| **골드 차감** | 납부 시 페르소나 소지금에서 세액(+연체이자) **실제 차감** |
...
- **페르소나 전용**: 페르소나가 활성화된 상태에서만 소득 자동 기록
- 협회/길드 정산, 직접 판매, 팀 분배 시 자동 기록
- 기록 필드: 날짜, 제목, 총수익, 수수료, 순수익, 지급액, 유형(협회/길드), 참가자(페르소나 이름)
- 게임 날짜(`gameDate`)를 기준으로 전월/당월 구분
- 소득기록 패널은 접기/펼치기 가능 (`incomeLogOpen` 상태)
